...
public class S_VFXObjectManager : MonoBehaviour
{
	[Serializable]
	public struct VFXSources {
		public string name;
		public ParticleSystem source;
	}

	public VFXSources[] _Sources;

	public void PlaySource ( string name, int index = -1 ) {
		ParticleSystem Source = GetSource(name, index);
	...
	private ParticleSystem GetSource ( string name, int index ) {
		if(_Sources == null || _Sources.Length == 0) { return null; }

		if (name != "")
		{
			foreach (VFXSources source in _Sources)
			{
				if (source.name == name)
