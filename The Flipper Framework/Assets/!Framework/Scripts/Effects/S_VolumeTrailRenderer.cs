...
public class S_VolumeTrailRenderer : MonoBehaviour
{
	...
	public Gradient baseColorOverLifeTime;
	public Gradient specialColorOverLifeTime;
	private Gradient inUseColorOverLifeTime;
		...
		if (vertices.Count > 1)
		{
			...
			for (int p = 0 ; p < vertices.Count ; p++)
			{
				...
				for (int c = 0 ; c < crossSegments ; c++)
				{
					...
					Color color = inUseColorOverLifeTime.Evaluate(overlifetime) * fadeBias;
