...
public class S_Control_EffectsPlayer : MonoBehaviour {
    ...
    void FixedUpdate () {
		...
		if(Player._RB.velocity.sqrMagnitude > RunningDustThreshold && Player._isGrounded && RunningDust != null)
        {
            RunningDust.Emit(Random.Range(0,20));
        }

		if (Player._RB.velocity.sqrMagnitude > SpeedLinesThreshold && Player._isGrounded && SpeedLines != null && SpeedLines.isPlaying == false) 
		{
			SpeedLines.Play ();
		} 
		else if (Player._RB.velocity.sqrMagnitude < SpeedLinesThreshold && SpeedLines.isPlaying == true || (!Player._isGrounded)) 
		{
			SpeedLines.Stop ();
		}
