...
public class S_PlayerPhysics : MonoBehaviour
{
	...
	void Update () {
		_speedMagnitude = rb.velocity.magnitude;
	...
	Vector3 HandleGroundControl ( float deltaTime, Vector3 input ) {
		if (_Action.whatAction != S_Enums.PlayerStates.JumpDash && _Action.whatAction != S_Enums.PlayerStates.WallRunning)
		{
			...
			if (input.sqrMagnitude != 0.0f)
			{
				...
				turnRate *= _turnRateOverSpeed_.Evaluate((rb.velocity.sqrMagnitude / _currentMaxSpeed) / _currentMaxSpeed);
	...
	private Vector3 StickToGround ( Vector3 Velocity ) {
		...
		if (_TimeOnGround > 0.1f && _speedMagnitude > 1)
		{
			...
			if (Physics.Raycast(Raycasterpos, rb.velocity.normalized, out _hitSticking, _speedMagnitude * _stickCastAhead_ * Time.deltaTime, _Groundmask_))
			{
			...
			else
			{
				if (Mathf.Abs(DirectionDot) < _stickingNormalLimit_) //avoid SuperSticking
				{
					...
					if (Physics.Raycast(Raycasterpos + (rb.velocity * _stickCastAhead_ * Time.deltaTime), -groundHit.normal, out _hitSticking, 2.5f, _Groundmask_))
					{
						float dist = _hitSticking.distance;
						if (_EnableDebug)
						{
							Debug.Log("PlacedDown" + dist);
							Debug.DrawRay(Raycasterpos + (rb.velocity * _stickCastAhead_ * Time.deltaTime), -groundHit.normal * 3, Color.cyan, 2);
						}
						if (dist > 1.5f)
						{
	...
	void AirMovement () {
		...
		//Max Falling Speed
		if (rb.velocity.y < _maxFallingSpeed_)
		{
