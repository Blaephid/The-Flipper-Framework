	...
	public void SetOnRail ( bool isTrigger, Collider collider, Vector2 rangeAndDis ) {
		...
		if (collider.gameObject.GetComponentInParent<Spline>())
		{
			...
			if (isTrigger)
			{
				...
				{
					offSet = collider.gameObject.GetComponentInParent<S_PlaceOnSpline>()._mainOffset;
				}
			}
			...
			else
			{
				...
				{
					offSet = collider.gameObject.GetComponentInParent<S_SplineMeshTiling>().translation;
				}
			}
