//
// parallax mapped material with adjustable features
// Tomasz Stobierski 2013-2016
//
Shader "Relief Pack - Bonus Shaders/Customizable Shader 1 Layer" {
    Properties {
		//
		// features
		//
		[RTP_BlockInfo(1.0,1.0,1.0,1.0, 1)] block_features("Features", Float) = 1 
			...
			[RTP_HideByProp(block_features,RTP_EMISSION)]
				...
				[RTP_EndArea(true)]
			[RTP_HideByProp()]
			[RTP_HideByProp(COLOR_MAP,block_features)]
				[RTP_BeginArea(none,true)] dummy_globaltextiling("", Float) = 0
		...
		[RTP_BlockInfo(1.0,1.0,0.5,0.8, 1)] block_layerprops("Layer properties", Float) = 1
			[RTP_HideByProp(block_layerprops)]
				[RTP_MaterialProp] _MainTex ("Texture (A - smoothness)", 2D) = "white" {}
				[RTP_MaterialProp(miniThumb)] _BumpMap ("Bumpmap", 2D) = "bump" {}
				[RTP_MaterialProp(miniThumb)] _HeightMap ("Heightmap (A)", 2D) = "black" {}
				[RTP_BeginArea(Box,true)] dummy_layerPBR("", Float) = 0
					[RTP_Header(PBR props)]
					[RTP_MaterialProp] _Metalness0("  Metalness", Range(0, 1)) = 0
					[RTP_MaterialProp] _GlossMin0("  Gloss Min", Range(0, 1)) = 0
					[RTP_MaterialProp] _GlossMax0("  Gloss Max", Range(0, 1)) = 1
					[RTP_MaterialProp] _FarSpecCorrection0("  far spec correction", Range(-1, 1)) = 0
					[RTP_MaterialProp] RTP_DiffFresnel0("  diffuse scattering", Range(0, 1)) = 0
				[RTP_EndArea(true)]
				[RTP_BeginArea(Box,true)] dummy_layerCorr("", Float) = 0
					[RTP_Header(Aux props)]
						[RTP_MaterialProp(noalpha)] _LayerColor0("Color tint", Color) = (0.5, 0.5, 0.5, 1)
						[RTP_MaterialProp] _LayerSaturation0("Saturation", Range(0, 2)) = 1
				[RTP_EndArea(true)]
			[RTP_HideByProp()]
			...
			[RTP_HideByProp(RTP_UV_BLEND, block_layerprops)]
				[RTP_BeginArea(Box, true)] dummy_layerUVBlend("", Float) = 0
					[RTP_Header(UV Blend)]
					[RTP_MaterialProp] _MixBlend0("UV blend strength", Range(0, 1)) = 0.5
					[RTP_MaterialProp] _MixScale0("  tiling", Range(0.02, 0.25)) = 0.125
					[RTP_MaterialProp] _MixSaturation0("  saturation", Range(0, 2)) = 1
					[RTP_MaterialProp] _MixBrightness0("  brightness", Range(0.5, 3.5)) = 2
					[RTP_MaterialProp] _MixReplace0("  replace at far", Range(0, 1)) = 0.25
				[RTP_EndArea(true)]
			[RTP_HideByProp()]
			...
			[RTP_HideByProp(RTP_EMISSION,block_layerprops)]
				[RTP_BeginArea(Box,true)] dummy_layerEmission("", Float) = 0
					[RTP_Header(Emission)]
					[RTP_MaterialProp] _LayerEmission0("Emission", Range(0, 1)) = 0
					[RTP_MaterialProp] _LayerEmissionColor0("  glow color", Color) = (0.5, 0.5, 0.5, 0)
				[RTP_EndArea(true)]
			[RTP_HideByProp()]
		...
		[RTP_BlockInfo(0.9,0.95,1.0,1.0, 1)] block_perlin("Perlin (amp) far distance properties", Float) = 1
			...
			[RTP_HideByProp(GLOBAL_PERLIN,block_perlin)]
				[RTP_BeginArea(Box,true)] dummy_perlin("", Float) = 0
					...
					[RTP_MaterialProp] _BumpMapGlobalStrength0("Perlin normal strength", Range(0,2)) = 0.3
		...
		[RTP_BlockInfo(1.0,1.0,1.0,1.0, 1)] block_snow("Snow settings", Float) = 1 
			...
			[RTP_HideByProp(RTP_SNOW, block_snow)]
				[RTP_BeginArea(Box,true)] dummy_snow("", Float) = 0
					...
					//[RTP_MaterialProp] rtp_snow_deep_factor("  Deep factor", Range(0, 6)) = 2
		...
		[RTP_BlockInfo(1.0,0.4,0.2,1.0, 1)] block_vert_tex("Vertical texturing", Float) = 1 
			...
			[RTP_HideByProp(VERTICAL_TEXTURE, block_vert_tex)]
				[RTP_BeginArea(Box,true)] dummy_vert_tex("", Float) = 0
					...
					[RTP_MaterialProp] _VerticalTextureStrength0("  Strength", Range(0,1)) = 0.5
				[RTP_EndArea(true)]
			[RTP_HideByProp()]
		...
		[RTP_BlockInfo(0.5,1.0,1.0,1.0, 1)] block_glitter("Glitter settings", Float) = 1 
			...
			[RTP_HideByProp(RTP_GLITTER, block_glitter)] 
				[RTP_BeginArea(Box,true)] dummy_glitter("", Float) = 0
					...
					[RTP_MaterialProp] _GlitterStrength0("Glitter strength", Float) = 1
	...
	CGPROGRAM
	#pragma surface surf Standard vertex:vert exclude_path:prepass
