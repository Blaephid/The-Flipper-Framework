...
//
Shader "Relief Pack - GeometryBlend/Water/  Basic" {
    Properties {
		...
		TERRAIN_WaterColor ("Water Color (A - opacity)", Color) = (1,1,1,0)
      	TERRAIN_WaterGloss ("Water Smoothness", Range(0,1)) = 0.95
	...
	void surf (Input IN, inout SurfaceOutputStandard o) {
		...
		float4 global_color_value=tex2D(_ColorMapGlobal, IN.custUV.zw);
		#ifdef SIMPLE_FAR
			global_color_value.rgb=lerp(dot(global_color_value.rgb,0.35).xxx, global_color_value.rgb, lerp(_GlobalColorMapSaturation,1,_uv_Relief_w));
		#else
      	...
      	float3 rayPos;
      	rayPos.z=tex2D(_HeightMap, IN.custUV.xy).a;
      	float control=(rayPos.z+0.01);
      	#if defined(RTP_SIMPLE_SHADING)
      	rayPos.xy=IN.custUV.xy;
      	#else
      	rayPos.xy=IN.custUV.xy + ParallaxOffset(rayPos.z, TERRAIN_ExtrudeHeight, IN.viewDir.xyz);
      	#endif
