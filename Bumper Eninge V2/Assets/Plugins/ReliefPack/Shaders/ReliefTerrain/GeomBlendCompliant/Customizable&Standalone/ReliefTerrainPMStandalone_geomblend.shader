...
//
// geom blend version - good for geom blend parts with most demanding quality (most RTP features, including POM shading available)
...
//
//
Shader "Relief Pack - GeometryBlend/ Standalone" {
	Properties {
		// selectable shading level via multicompile keywords
		[KeywordEnum(POM_Shading_Hi, POM_Shading_Lo, PM_Shading)] rtp("Shading level", Float) = 0
		//
		// features
		//
		...
		[RTP_BlockInfo(1.0,0.5,0.5,0.8, 1)] block_main("Main settings", Float) = 1 
			...
			[RTP_HideByProp(block_main, rtp.0.1)]
				[RTP_BeginArea(Box,true)] dummy_POM("", Float) = 0
			[RTP_HideByProp()]
			[RTP_HideByProp(block_main, rtp.0.1)]
					[RTP_Header(POM)]
					[RTP_MaterialProp] _TERRAIN_DIST_STEPS("Color max search steps", Range(4,255)) = 20
					[RTP_MaterialProp] _TERRAIN_WAVELENGTH("   step length (in texels)", Range(0.5,16)) = 2
			[RTP_HideByProp()]
			[RTP_HideByProp(block_main, rtp.0)]
					[RTP_Header(POM self(minus)shadowing)]
					[RTP_MaterialProp] _TERRAIN_SHADOW_STEPS("Self-shadow max search steps", Range(0,80)) = 20
					[RTP_MaterialProp] _TERRAIN_WAVELENGTH_SHADOWS("   step length (in texels)", Range(0.5,16)) = 2
					[RTP_MaterialProp] _TERRAIN_SelfShadowStrength("   strength", Range(0,1)) = 0.8
					[RTP_MaterialProp] _TERRAIN_ShadowSmoothing("   smoothing", Range(0,1)) = 1
					[RTP_MaterialProp] _TERRAIN_ShadowSoftnessFade("   softness fade", Range(0.2, 1.5)) = 1
			[RTP_HideByProp()]
			[RTP_HideByProp(block_main, rtp.0.1)]
				[RTP_EndArea(true)]
			[RTP_HideByProp()]
	...
	SubShader {
		...
		#pragma multi_compile RTP_POM_SHADING_HI RTP_POM_SHADING_LO RTP_PM_SHADING
