...
{
internal class StandardShaderGUI_RTPGeomBlend : ShaderGUI
{
	...
	public void ShaderPropertiesGUI (Material material)
	{
		...
		EditorGUI.BeginChangeCheck();
		{
			...
			m_MaterialEditor.ShaderProperty(_TERRAIN_PosSize, Styles._TERRAIN_PosSizeText.text);
			m_MaterialEditor.ShaderProperty(_TERRAIN_Tiling, Styles._TERRAIN_TilingText.text);			
		}
		if (EditorGUI.EndChangeCheck())
		{
...
//		}
	...
	// RTP - geom blend - we use only transparent (fade)
	void BlendModePopup()
	{
...
//		var mode = (BlendMode)blendMode.floatValue;
		...
		EditorGUILayout.Popup(Styles.renderingMode, (int)BlendMode.Fade, Styles.blendNames);
		EditorGUI.EndDisabledGroup();
		if (!blendMode.hasMixedValue) {
			if (blendMode.floatValue!=(float)BlendMode.Fade) {
				blendMode.floatValue=(float)BlendMode.Fade;
				MaterialChanged((Material)m_MaterialEditor.target, m_WorkflowMode);
			}
		}
//		EditorGUI.BeginChangeCheck();
