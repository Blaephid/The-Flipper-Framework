...
// RTP - geom blend - posWorld always available for mapping
...
#define FRAGMENT_SETUP(x) FragmentCommonData x = \
	FragmentSetup(i.tex, i.eyeVec, IN_VIEWDIR4PARALLAX(i), i.tangentToWorldAndParallax, i.posWorld, i.color); // RTP - geom blend - vertex color added, world pos always available

#define FRAGMENT_SETUP_FWDADD(x) FragmentCommonData x = \
	FragmentSetup(i.tex, i.eyeVec, IN_VIEWDIR4PARALLAX_FWDADD(i), i.tangentToWorldAndLightDir, i.posWorld, i.color); // RTP - geom blend - vertex color added, world pos always available
...
inline FragmentCommonData FragmentSetup (float4 i_tex, half3 i_eyeVec, half3 i_viewDirForParallax, half4 tangentToWorld[3], half3 i_posWorld, fixed4 i_vertex_color) // RTP - geom blend - vertex color added
{
	// RTP - take height value for blending, damp parallax on blended areas by vertex color.a
	half tH=0.5;
	i_tex = Parallax(i_tex, i_viewDirForParallax, 1-i_vertex_color.a, /* inout */ tH);
	...
	half alpha; //=Alpha(i_tex.xy);
		...
		float4 splat_control1=terrain_coverage * tex2D(_TERRAIN_HeightMap, aux.xy) * i_vertex_color.a;
		float4 splat_control2=float4( (tH+0.01) , 0, 0, 0) * (1-i_vertex_color.a);
		...
		if (blend_coverage>0.1) {
			...
			alpha=lerp(1-i_vertex_color.a, alpha, saturate((blend_coverage-0.1)*4) );
		} else {
			alpha=1-i_vertex_color.a;
		}
	#else
		alpha=(1-i_vertex_color.a);
	#endif
...
struct VertexOutputForwardBase
{
	...
	fixed4 color						: COLOR0; // RTP - geom blend - vertex color added
	...
	//#if UNITY_SPECCUBE_BOX_PROJECTION
		float3 posWorld					: TEXCOORD8; // RTP - geom blend - always available for mapping
	//#endif
...
#endif
	float3 posWorld						: TEXCOORD9; // RTP - geom blend - always available for mapping
...
half4 fragForwardAddInternal (VertexOutputForwardAdd i)
{
	...
	//UnityLight light = AdditiveLight (s.normalWorld, IN_LIGHTDIR_FWDADD(i), LIGHT_ATTENUATION(i)); // (deprecated LIGHT_ATTENUATION(i) macro)
...
//	#if UNITY_SPECCUBE_BOX_PROJECTION
		float3 posWorld					: TEXCOORD6; // RTP - geom blend - always available for mapping
//	#endif
	#if UNITY_OPTIMIZE_TEXCUBELOD
		//#if UNITY_SPECCUBE_BOX_PROJECTION
