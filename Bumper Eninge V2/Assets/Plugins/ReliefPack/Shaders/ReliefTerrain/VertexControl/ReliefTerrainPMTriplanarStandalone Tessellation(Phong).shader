...
//
//
Shader "Relief Pack - Standalone/Parallax Mapped Triplanar with Tessellation(Phong)" {
	Properties {
		//
		// features
		//
		...
		// tessellation
		//
		[RTP_BlockInfo(1.0,1.0,1.0,1.0, 1)] block_tessprops("Tessellation properties", Float) = 1
			[RTP_HideByProp(block_tessprops)]
				[RTP_BeginArea(Box, false)] dummy_tessprops("", Float) = 0
					[RTP_Header()]
					[RTP_MaterialProp] _Phong ("Phong smooth strength", Range(0,1)) = 0.7
					[RTP_MaterialProp] _TessSubdivisions ("Subdivisions - close", Range(1,32)) = 3
					[RTP_MaterialProp] _TessSubdivisionsFar ("Subdivisions - far", Range(1,32)) = 3
				[RTP_EndArea(false)]
			[RTP_HideByProp()]
	...
	SubShader {
		...
		#pragma surface surf Standard vertex:vert finalcolor:customFog exclude_path:prepass tessellate:tessEdge tessphong:_Phong fullforwardshadows 
		// U5 fog handling
...
//		#define WNORMAL_COVERAGE_XZ_YposYneg
		...
		// below switch is meant to be used with new TerraVol LOD system
		// we can interpolate between vertex position set into vertices buffer in mesh and 2nd set of vertices positions set into (uv.x, uv.y, uv2.x)
		// this way we can smoothly morph between LOD levels
		//#define INTERPOLATE_VERTICES
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// below options, however possible to use, are mostly impractical for usage in this shader (global maps would be mapped topplanar basing on _TERRAIN_PosSize shader property)
//
