...
[ExecuteInEditMode]
public class AddOnRail : MonoBehaviour
{
    [Header("Updating in Editor")]
    public bool UpdateAll = false;
    public bool UpdateNow = false;
    public AddOnRail[] AddThis;
    public AddOnRail[] AddBehindThese;

    [Header("Main Rails")]
    public bool setConnectedOfOtherObjects;
    public AddOnRail nextRail;
    public AddOnRail PrevRail;

    [Header("Switch Rails")]
    public AddOnRail altNextRail;
    public AddOnRail altPrevRail;
    ...
    public void switchTrigger()
    {
        if(altNextRail != null)
        {
            AddOnRail temp = nextRail;
            nextRail = altNextRail;
            altNextRail = temp;
        }
        if(altPrevRail != null)
        {
            AddOnRail temp = PrevRail;
    ...
    private void Update()
    {
        ...
        if(UpdateAll)
        {
            AddOnRail[] railsMeshes = FindObjectsOfType<AddOnRail>();
            foreach (AddOnRail add in railsMeshes)
            {
    ...
    void Place()
    {
        offset = new Vector3 (GetComponent<ExampleSower>().Offset3d.x, 0, 0);
        //offset = Vector3.zero;

        if (AddThis.Length > 0)
        {
            foreach (AddOnRail rail in AddThis)
            {
            ...
            foreach(AddOnRail rail in AddBehindThese)
            {
                Spline thisSpline = GetComponentInParent<Spline>();
                Spline otherSpline = rail.GetComponentInParent<Spline>();
